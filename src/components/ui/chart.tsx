...
>(
  (
    ...
    const tooltipLabel = React.useMemo(() => {
      ...
      const [item] = payload
      const key = `${labelKey || "dateFormatted"}`
    ...
    return (
      <div
        ref={ref}
        className={cn(
          "grid min-w-[8rem] items-start gap-1.5 rounded-lg border bg-card p-2.5 text-sm shadow-xl",
          className
        )}
      >
        {tooltipLabel}
            ...
            >
              {itemConfig?.icon && !hideIcon ? (
                ...
                <div
                  ...
                  style={{
                    backgroundColor: color.startsWith('url') ? 'hsl(var(--primary))' : color,
                  }}
                />
              )}
              {itemConfig?.label}
